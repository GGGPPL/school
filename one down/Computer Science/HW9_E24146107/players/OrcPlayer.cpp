...
OrcPlayer::OrcPlayer() : NovicePlayer() { setLevel(1); setHp(maxhp); setMp(maxmp); }
OrcPlayer::OrcPlayer(int l) : NovicePlayer(l) { setLevel(l); setHp(maxhp); setMp(maxmp); }
OrcPlayer::OrcPlayer(int l, string n) : NovicePlayer(l, n) { setLevel(l); setHp(maxhp); setMp(maxmp); }
OrcPlayer::OrcPlayer(const OrcPlayer& other) : NovicePlayer(other) {}

void OrcPlayer::setLevel(int l) {
    level = l; maxhp = 200 + 20 * level; maxmp = 50 + 5 * level;
    attack = 50 + 12 * level; defense = 30 + 10 * level;
